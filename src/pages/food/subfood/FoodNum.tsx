...
interface NumFoodProps{
  foodsNum: FoodItemType[] 
}

const FoodNum:React.FC<NumFoodProps> = ({ foodsNum}) => {
  const [data, setData] = useState<FoodItemType[]>([]);
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [num, setNum] = useState<number>(0);

  const handleFilter = () => {
    let filtered = foodsNum;
    if (searchTerm !== "") {
  ...
  return (
    <>
      <ProductHeader
        ...
        noneImg={""}
      />
      <NavLink numKhmer={true} handleSearch={setSearchTerm} />
